//
//  AccountDetailDataManipulator.swift
//  StatementList
//
...
class AccountDetailDataManipulator: NSObject {
    ...
    public func setupTableView(_ tableView: UITableView, presentationDelegate:UIViewController) {
        ...
        let accountDetailHeader: AccountDetailHeaderCell = tableView.dequeueReusableCell(withIdentifier: Identifiers.Cell.accountDetailHeader) as! AccountDetailHeaderCell
    ...
    // latest 7 days spending + 30 days average spending * 7
    // This way the next 14 days spending won't be affected too much if there is a huge non-everyday spending in last 30 days
    public func predictedSpending () -> NSNumber {
        let spending = self.spendingIn(days: 7) + self.spendingIn(days: 30) / 30 * 7
        return NSNumber(value: spending)
    }
    
    // Find the last date and count back (days: Int) days then collect all spending within that time range.
...
extension AccountDetailDataManipulator: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let transactionCell: TransactionCell = tableView.dequeueReusableCell(withIdentifier: Identifiers.Cell.transactionCell) as! TransactionCell
...
extension AccountDetailDataManipulator: UITableViewDelegate {
    
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let sectionHeader: DateSectionHeaderCell = tableView.dequeueReusableCell(withIdentifier: Identifiers.Cell.dateSectionHeader) as! DateSectionHeaderCell
    ...
    // If an ATM location is found in current transaction, 
    // create ATMLocationViewController and push it in.
